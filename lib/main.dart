...
import 'package:todo_app/application/service/background_service_registry.dart';
import 'package:todo_app/application/service/service_port_names.dart';
...
void callbackDispatcher() {
  Workmanager().executeTask((task, inputData) async {
    ...
    await BackgroundServiceRegistry.init();
    await BackgroundServiceRegistry.addAll((GetIt getIt) {
      return [
        // add all functions that need to be executed in background
        (getIt<TodoRepository>().sync(), ServicePortNames.todo),
        (getIt<UserSettingsRepository>().sync(), ServicePortNames.userSettings),
      ];
    });
    return true;
  });
